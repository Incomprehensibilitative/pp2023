...
class Management:
    ...
    def new_student(self):
        ...
        for i in range(n):
            ...
            if ip.validate_id(student.get_id(), self.__student_list):
                ...
                break
            thread = th.Thread(target=self.get_pickled, args=(student, "students", "ab+"))
            thread.start()
            thread.join()
    ...
    def new_course(self):
        ...
        for i in range(n):
            ...
            thread = th.Thread(target=self.get_pickled, args=(course, "courses", "ab+"))
            thread.start()
            thread.join()
    ...
    def new_mark(self):
        ...
        thread = th.Thread(target=self.get_pickled, args=(marking, "marks", "ab+"))
        thread.start()
        thread.join()
